...
Matrix multiply_navie(const Matrix &a, const Matrix &b){
    Matrix c;
    for(int i = 0; i < N; i++){
        for(int j = 0; j < N; j++){
            for(int k = 0; k < N; k++){
...
double benchmark(Matrix (*mutiply)(const Matrix&, const Matrix&), Matrix &result){
...
int main(){
    ...
    Matrix a, b;
    printf("Slow version\n");
    double slow = benchmark(multiply_navie, a);
    printf("Fast version\n");
